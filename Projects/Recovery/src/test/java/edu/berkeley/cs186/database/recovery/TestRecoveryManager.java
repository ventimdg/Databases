...
public class TestRecoveryManager {
    ...
    @Test
    @Category(PublicTests.class)
    public void testMassiveCheckpoint() {
        // Create 765 DPT entries and 765 transaction table entries
        for (long l = 1; l <= 765; l++) {
            dirtyPageTable.put(l, l*l);
        ...
        LogRecord endCheckpoint4 = logs.next();
        LogRecord endCheckpoint5 = logs.next();
        LogRecord endCheckpoint6 = logs.next();
        LogRecord endCheckpoint7 = logs.next();
        assertEquals(LogType.BEGIN_CHECKPOINT, beginCheckpoint.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint1.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint2.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint3.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint4.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint5.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint6.getType());
        assertEquals(LogType.END_CHECKPOINT, endCheckpoint7.getType());
        assertFalse(logs.hasNext());
        ...
        // Second end checkpoint should have no DPT entries, and the remaining
        // transaction table entries.
        assertEquals(255, endCheckpoint2.getDirtyPageTable().size());
        assertEquals(0, endCheckpoint2.getTransactionTable().size());

        assertEquals(255, endCheckpoint3.getDirtyPageTable().size());
        assertEquals(0, endCheckpoint3.getTransactionTable().size());

        assertEquals(0, endCheckpoint4.getDirtyPageTable().size());
        assertEquals(240, endCheckpoint4.getTransactionTable().size());

        assertEquals(0, endCheckpoint5.getDirtyPageTable().size());
        assertEquals(240, endCheckpoint5.getTransactionTable().size());

        assertEquals(0, endCheckpoint6.getDirtyPageTable().size());
        assertEquals(240, endCheckpoint6.getTransactionTable().size());

        assertEquals(0, endCheckpoint7.getDirtyPageTable().size());
        assertEquals(45, endCheckpoint7.getTransactionTable().size());
        ...
        for (long l = 1; l <= 765; l++) {
            ...
            }else if (endCheckpoint4.getTransactionTable().containsKey(l)) {
                p = endCheckpoint4.getTransactionTable().get(l);
            }else if (endCheckpoint5.getTransactionTable().containsKey(l)) {
                p = endCheckpoint5.getTransactionTable().get(l);
            }else if (endCheckpoint6.getTransactionTable().containsKey(l)) {
                p = endCheckpoint6.getTransactionTable().get(l);
            }else if (endCheckpoint7.getTransactionTable().containsKey(l)) {
                p = endCheckpoint7.getTransactionTable().get(l);
            } else {
                fail("Transaction #" + l + " could not be found in checkpoint ttables");
                return;
            }
    ...
    @Test
    @Category(PublicTests.class)
    public void testRestartRedoEmpty() {
        byte[] before = new byte[] { (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00 };
        byte[] after = new byte[] { (byte) 0xBA, (byte) 0xAD, (byte) 0xF0, (byte) 0x0D };
        DiskSpaceManager dsm = diskSpaceManager;
        BufferManager bm = bufferManager;
    ...
    @Test
    @Category(PublicTests.class)
    public void testRestartUndoEmptyLog() {
        byte[] before = new byte[] { (byte) 0x00, (byte) 0x00, (byte) 0x00, (byte) 0x00 };
        byte[] after = new byte[] { (byte) 0xBA, (byte) 0xAD, (byte) 0xF0, (byte) 0x0D };
        DummyTransaction transaction1 = DummyTransaction.create(1L);
        ...
        LSNs.add(logManager
                .appendToLog(new UpdatePageLogRecord(1L, 10000000004L, LSNs.get(2), (short) 3, before, after))); // 3
        LSNs.add(logManager.appendToLog(new CommitTransactionLogRecord(1L, LSNs.get(3)))); // 4
