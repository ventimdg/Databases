...
public class TestLockContext {
    ...
    public void testPromoteSIXSaturation() {
        ...
        runner.run(0, () -> r0.acquire(t1, LockType.IX));
        runner.run(0, () -> r1.acquire(t1, LockType.IS));
        runner.run(0, () -> r2.acquire(t1, LockType.S));
        runner.run(0, () -> r0.promote(t1, LockType.SIX));
        assertEquals(r0.getNumChildren(t1), 0);
        assertEquals(r1.getNumChildren(t1), 0);
        assertEquals(r2.getNumChildren(t1), 0);

    }
    @Test
    public void testPromoteSIXSaturation2() {
        TransactionContext t1 = transactions[1];
        LockContext r0 = dbLockContext;
        LockContext r1 = tableLockContext;
        LockContext r2 = pageLockContext;
        ...
        DeterministicRunner runner = new DeterministicRunner(1);
        runner.run(0, () -> r0.acquire(t1, LockType.IX));
        runner.run(0, () -> r1.acquire(t1, LockType.IX));
        runner.run(0, () -> r2.acquire(t1, LockType.IS));
        runner.run(0, () -> r0.promote(t1, LockType.SIX));
        assertEquals(r0.getNumChildren(t1), 1);
        assertEquals(r1.getNumChildren(t1), 0);
        assertEquals(r2.getNumChildren(t1), 0);
