...
public class SortOperator extends QueryOperator {
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        Run answer = makeRun();
        ArrayList<ArrayList<Record>> newRuns = new ArrayList<>();
        ...
        for (int i = 0; i < newRuns.size(); i++){
            Record min = Collections.min(newRuns.get(i), comparator);
            q.add(new Pair<>(min, i));
            newRuns.get(i).remove(min);
        }

        while(q.peek() != null) {
            Pair<Record, Integer> nextMerge = q.poll();
            Record minRecord = nextMerge.getFirst();
            int runNumber = nextMerge.getSecond();
            answer.add(minRecord);
            if (newRuns.get(runNumber).size() > 0){
                Record min = Collections.min(newRuns.get(runNumber), comparator);
                q.add(new Pair<>(min, runNumber));
                newRuns.get(runNumber).remove(min);
            }
        }
        return answer;
    }
    ...
    public List<Run> mergePass(List<Run> runs) {
        // TODO(proj3_part1): implement
        List<Run> answer = new ArrayList<>();
        List<List<Run>> subRun = new ArrayList<>();
        for(int i = 0; i < runs.size(); i += numBuffers-1){
            List<Run> temp = new ArrayList<>();
            for(int j = i; j < i + numBuffers - 1; j++){
                if (j >= runs.size()){
                    break;
                } else {
                    temp.add(runs.get(j));
        ...
        for(List<Run> run : subRun){
            answer.add(mergeSortedRuns(run));
        }
        return answer;
    }
    ...
    public Run sort() {
        ...
        while(sourceIterator.hasNext()){
            BacktrackingIterator<Record> temp = QueryOperator.getBlockIterator(sourceIterator, getSource().getSchema(), numBuffers);
            runList.add(sortRun(temp));
        }
        while(runList.size() > 1){
            runList = mergePass(runList);
        }
        return runList.size() > 0 ? runList.get(0) : null; // TODO(proj3_part1): replace this!
    }
