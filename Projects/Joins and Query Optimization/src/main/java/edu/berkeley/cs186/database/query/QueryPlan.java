...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int minOpIndex = -1;
        int IOs  = minOp.estimateIOCost();

        List<Integer> columns = getEligibleIndexColumns(table);

        for  (int cols : columns){
            SelectPredicate predicate = this.selectPredicates.get(cols);
            QueryOperator scan = new IndexScanOperator(
                    this.transaction,
                    table,
                    predicate.column,
                    predicate.operator,
            ...
            int tempIO = scan.estimateIOCost();
            if (tempIO < IOs){
                IOs = tempIO;
                minOp = scan;
                minOpIndex = cols;
...
//        QueryOperator seqScan = new SequentialScanOperator(this.transaction, table);
//        seqScan = addEligibleSelections(seqScan, -1);
//        int seqIO  = seqScan.estimateIOCost();
//
//        QueryOperator indexScan = null;
//        int indexIO = Integer.MAX_VALUE;
//
//        List<Integer> columns = getEligibleIndexColumns(table);
//
//        for  (int cols : columns){
...
//                    predicate.value
//            );
//            scan = addEligibleSelections(scan, cols);
//            int tempIO = scan.estimateIOCost();
//            if (tempIO < seqIO && tempIO < indexIO){
//                indexIO = tempIO;
//                indexScan = scan;
//            }
//        }
...
//        return seqScan;
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        Map<Set<String>, Integer> joinMins = new HashMap<>();
        for (Set<String> tables : prevMap.keySet()){
            for (JoinPredicate jp : this.joinPredicates) {
                ...
                if (tables.contains(leftTable) && !tables.contains(rightTable)){
                    ...
                    leftOp = prevMap.get(tables);
                    newResult = new HashSet<>(tables);
                    newResult.add(leftTable);
                    String temporary = leftCol;
                    leftCol = rightCol;
                    rightCol = temporary;
                ...
                int IOcost = minJoin.estimateIOCost();

                if (!joinMins.containsKey(newResult)){
                    result.put(newResult, minJoin);
                    joinMins.put(newResult, IOcost);

                } else if (IOcost < joinMins.get(newResult)) {
                    joinMins.replace(newResult, IOcost);
                    result.replace(newResult, minJoin);
                }
            }
        }
        return result;
    ...
    public Iterator<Record> execute() {
        ...
        for (String table: tableNames){
            ...
            cur.add(table);
            pass1.put(cur, minCostSingleAccess(table));
        }

        Map<Set<String>, QueryOperator> passX = minCostJoins(pass1, pass1);
        int i = 2;
        while(i < tableNames.size()){
            passX = minCostJoins(passX, pass1);
            i++;
        }

        if (passX.isEmpty()){
            this.finalOperator = minCostOperator(pass1);
        } else {
            this.finalOperator = minCostOperator(passX);
        }
        this.addGroupBy();
        this.addProject();
        this.addSort();
