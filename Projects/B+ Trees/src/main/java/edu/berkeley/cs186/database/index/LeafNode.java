...
class LeafNode extends BPlusNode {
    ...
    public Optional<Pair<DataBox, Long>> put(DataBox key, RecordId rid) {
        if (this.keys.contains(key)){
            throw new BPlusTreeException("Key already exits");
        }
//        int index = 0;
//        DataBox curkey =  null;
//
//        for(DataBox keys : this.keys){
//            if(keys.compareTo(key) < 0){
//                if (curkey == null){
//                    curkey = keys;
...
//        }
        ...
        //WE DO NOT KNOW IF THE PAGE NUMBER OF THIS PAIR IS CORRECT
        if(this.keys.size() > this.metadata.getOrder() * 2){

            int start = this.metadata.getOrder();
            ...
            LeafNode split = new LeafNode(this.metadata, this.bufferManager, newkeys, newrids, this.rightSibling, this.treeContext);
            this.rightSibling = Optional.of(split.page.getPageNum());
            sync();
            return Optional.of(new Pair<DataBox, Long>(split.keys.get(0), split.page.getPageNum()));
        }
        sync();
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        while(data.hasNext()){
            ...
            DataBox key = curr.getFirst();
            RecordId rid = curr.getSecond();
            if (this.keys.contains(key)){
                throw new BPlusTreeException("Key already exits");
            }
            ...
            if(this.keys.size() > capacity){
                ...
                List<DataBox> newkeys = new ArrayList<>();
                List<RecordId> newrids = new ArrayList<>();
                newkeys.add(keys.get(capacity));
                newrids.add(rids.get(capacity));
                keys.remove(capacity);
                rids.remove(capacity);
                LeafNode split = new LeafNode(this.metadata, this.bufferManager, newkeys, newrids, this.rightSibling, this.treeContext);
                this.rightSibling = Optional.of(split.page.getPageNum());
                sync();
                return Optional.of(new Pair<DataBox, Long>(split.keys.get(0), split.page.getPageNum()));
