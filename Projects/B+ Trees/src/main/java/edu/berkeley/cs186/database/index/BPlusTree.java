...
public class BPlusTree {
    ...
    public Iterator<RecordId> scanAll() {
        ...
        BPlusTreeIterator a = new BPlusTreeIterator(leftmost);
    ...
    public Iterator<RecordId> scanGreaterEqual(DataBox key) {
        ...
        BPlusTreeIteratorGE a = new BPlusTreeIteratorGE(leftmost, key);
    ...
    public void put(DataBox key, RecordId rid) {
        ...
        Optional<Pair<DataBox, Long>> bubble = root.put(key, rid);
        if (!bubble.equals(Optional.empty())) {
            DataBox insertedkey = bubble.get().getFirst();
            Long child = bubble.get().getSecond();
            ...
            InnerNode newInner = new InnerNode(this.metadata, this.bufferManager, keys, children, lockContext);
    ...
    public void bulkLoad(Iterator<Pair<DataBox, RecordId>> data, float fillFactor) {
        ...
        if (root.getLeftmostLeaf().getKeys().size() != 0){
            ...
            while (data.hasNext()){
                Optional<Pair<DataBox, Long>> bubble = root.bulkLoad(data, fillFactor);
                if (!bubble.equals(Optional.empty())) {
                    DataBox insertedkey = bubble.get().getFirst();
                    Long child = bubble.get().getSecond();
    ...
    private class BPlusTreeIterator implements Iterator<RecordId> {
        ...
        private int counter;
        public BPlusTreeIterator(LeafNode start) {
            this.start = start;
            this.counter = 0;
        }
        ...
        public boolean hasNext() {
            // TODO(proj2): implement
            if (counter >= start.getRids().size() && start.getRightSibling().equals(Optional.empty())){
                return false;
            }
            else if (counter >= start.getRids().size() && !start.getRightSibling().equals(Optional.empty())){
                LeafNode iter = start;
                while(!iter.getRightSibling().equals(Optional.empty())){
                    iter = iter.getRightSibling().get();
                    if (iter.getKeys().size() > 0){
                        return true;
                    }
                }
                return false;
        ...
        public RecordId next() {
            // TODO(proj2): implement
            if (counter < start.getRids().size()){
                RecordId a = start.getRids().get(counter);
                counter ++;
                return a;
            }
            else {
    ...
    private class BPlusTreeIteratorGE implements Iterator<RecordId> {
        // TODO(proj2): Add whatever fields and constructors you want here.
        private LeafNode start;
        private int counter;
        private DataBox checker;
        public BPlusTreeIteratorGE(LeafNode start, DataBox num) {
            this.start = start;
            this.counter = 0;
            this.checker = num;
            while (start.getKeys().size() > counter && checker.compareTo(start.getKeys().get(counter)) > 0){
                counter ++;
            }
        }
