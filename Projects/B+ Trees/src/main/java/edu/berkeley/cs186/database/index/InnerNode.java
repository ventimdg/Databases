...
//        }
        ...
        long child = this.children.get(index);
        BPlusNode next = BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, child);
        return next.get(key);
    }
    ...
    public LeafNode getLeftmostLeaf() {
        ...
        // TODO(proj2): implement
        BPlusNode next = BPlusNode.fromBytes(metadata, bufferManager, treeContext, this.children.get(0));
        return next.getLeftmostLeaf();
    }
...
//        }
        ...
        long childs = this.children.get(index);
        BPlusNode next = BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, childs);

        LeafNode work;
        Optional<Pair<DataBox, Long>> a;
        ...
        try {
            if (a.equals(Optional.empty())){
                ...
                DataBox insertedkey = a.get().getFirst();
                Long child = a.get().getSecond();

//                index = 0;
//                DataBox curkey =  null;

//                for(DataBox keys : this.keys){
//                    if(keys.compareTo(insertedkey) < 0){
//                        if (curkey == null){
//                            curkey = keys;
//                        } else if (keys.compareTo(curkey) > 0){
//                            curkey = keys;
//                        }
//                    }
//                }
//                if (curkey != null){
//                    index = this.keys.indexOf(curkey) + 1;
//                }
                index = InnerNode.numLessThanEqual(insertedkey, keys);
                this.keys.add(index, insertedkey);
                this.children.add(index + 1, child);

                if(this.keys.size() > this.metadata.getOrder() * 2){

                    List<DataBox> keys2 = new ArrayList<>(keys);
                    List<Long> children2 = new ArrayList<>(children);
                    for(int i = keys.size() - 1; i >= 0; i--){
                        if(i >= this.metadata.getOrder()){
                            keys.remove(i);
                            children.remove(i);
                        } else {
                            keys2.remove(i);
                    ...
                    children.add(children2.get(0));
                    children2.remove(0);
                    DataBox holder = keys2.get(0);
                    keys2.remove(0);

                    InnerNode yo = new InnerNode(this.metadata, this.bufferManager, keys2, children2, this.treeContext);
                    sync();
                    return Optional.of(new Pair<DataBox, Long>(holder, yo.page.getPageNum()));
                }
            }
        } catch (Exception e) {
            throw new BPlusTreeException("Error in Inner Node Put");
        }
        sync();
        return Optional.empty();
    }
    ...
    public Optional<Pair<DataBox, Long>> bulkLoad(Iterator<Pair<DataBox, RecordId>> data,
        ...
        long childs = this.children.get(children.size()-1);
        BPlusNode next = BPlusNode.fromBytes(this.metadata, this.bufferManager, this.treeContext, childs);

        Optional<Pair<DataBox, Long>> a = next.bulkLoad(data, fillFactor);
...
//                    index = this.keys.indexOf(curkey) + 1;
//                }
                int index = InnerNode.numLessThanEqual(insertedkey, keys);
